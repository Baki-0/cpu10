...
short reg[8];	//汎用レジスタ(Reg0からReg7)
short rom[256];	//プログラム領域rom
short ram[256];	//データ領域ram

void assembler(void);	//アセンブラ関数のプロトタイプ宣言

//アセンブラで使う関数のプロトタイプ宣言
short mov(short, short);
short add(short, short);
short sub(short, short);
short andd(short, short);
short orr(short, short);
short sl(short);
short sr(short);
short sra(short);
...
//インストラクションレジスタから命令コードやオペランドを抽出する関数のプロトタイプ宣言
short op_code(short);
short op_regA(short);
short op_regB(short);
short op_data(short);
...
int main(void) {
	...
	do {
		...
		switch (op_code(ir)) {
			...
			case AND: reg[op_regA(ir)] = reg[op_regA(ir)] & reg[op_regA(ir)];
				break;
...
short andd(short ra, short rb) {
	return((AND << 11) | (ra << 8) | (rb << 5));
}

short orr(short ra, short rb) {
